...
**_NOTE:_** As of August 2026, Google requires a BotGuard attestation token (the `X-Goog-BatchExecute-Bgr` header) on the flight search RPCs, and withholds hotel prices without it. The token is generated by browser-side JS and signs the exact request payload, so the direct-HTTP `flights.New()` session gets empty flight results and unpriced hotels. Use `flights.NewBrowserSession()` instead: it drives a headless Chrome/Chromium (auto-detected; override with the `CHROME_BIN` env var) that loads the real search page and captures the RPC responses. `SerializeURL`, `AbbrCity`, and the other non-search helpers still work on a plain `flights.New()` session.

```
session, err := flights.NewBrowserSession()
...
defer session.Close()
```
